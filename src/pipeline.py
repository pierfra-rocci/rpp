...
def cross_match_with_gaia(
    ...
    # Check if the filter_band column exists in the Gaia table
    if filter_band not in gaia_table.colnames:
        st.error(f"Filter band '{filter_band}' not found in Gaia table.")
        st.info(f"Available columns: {', '.join(gaia_table.colnames)}")
        
        # Try to use a fallback filter band
        fallback_bands = ['phot_g_mean_mag', 'phot_bp_mean_mag', 'phot_rp_mean_mag']
        filter_band_to_use = None
        
        for fallback in fallback_bands:
            if fallback in gaia_table.colnames:
                filter_band_to_use = fallback
                st.warning(f"Using fallback filter band: {fallback}")
                break
        
        if filter_band_to_use is None:
            st.error("No suitable magnitude column found in Gaia table.")
            return None
    else:
        filter_band_to_use = filter_band

    try:
        # Create magnitude filter with proper error handling
        mag_column = gaia_table[filter_band_to_use]
        valid_mag_mask = np.isfinite(mag_column) & (mag_column < filter_max_mag)
        
        if not np.any(valid_mag_mask):
            st.warning(f"No Gaia sources with valid magnitudes in {filter_band_to_use} < {filter_max_mag}")
            return None

        # Apply other quality filters with error handling
        quality_filters = {}
        
        # Variable star filter
        if "phot_variable_flag" in gaia_table.colnames:
            quality_filters["var_filter"] = gaia_table["phot_variable_flag"] != "VARIABLE"
        else:
            quality_filters["var_filter"] = np.ones(len(gaia_table), dtype=bool)
            st.info("phot_variable_flag not available, skipping variability filter")
        
        # Color index filter
        if "bp_rp" in gaia_table.colnames:
            bp_rp = gaia_table["bp_rp"]
            quality_filters["color_index_filter"] = np.isfinite(bp_rp) & (bp_rp > -1) & (bp_rp < 2)
        else:
            quality_filters["color_index_filter"] = np.ones(len(gaia_table), dtype=bool)
            st.info("bp_rp not available, skipping color index filter")
        
        # Astrometric quality filter
        if "ruwe" in gaia_table.colnames:
            ruwe = gaia_table["ruwe"]
            quality_filters["astrometric_filter"] = np.isfinite(ruwe) & (ruwe < 1.6)
        else:
            quality_filters["astrometric_filter"] = np.ones(len(gaia_table), dtype=bool)
            st.info("ruwe not available, skipping astrometric quality filter")

        # Combine all filters
        combined_filter = (valid_mag_mask & 
                          quality_filters["var_filter"] & 
                          quality_filters["color_index_filter"] & 
                          quality_filters["astrometric_filter"])

        gaia_table_filtered = gaia_table[combined_filter]

        if len(gaia_table_filtered) == 0:
            st.warning(
                f"No Gaia sources found within magnitude range {filter_band_to_use} < {filter_max_mag} after filtering."
            )
            return None
    ...
    try:
        ...
        # Add the magnitude column that was actually used
        gaia_mags = gaia_table_filtered[filter_band_to_use][matched_indices_gaia]
        matched_table[filter_band_to_use] = gaia_mags
        
        # If we used a fallback band, also store the original requested band name
        if filter_band_to_use != filter_band:
            matched_table[filter_band] = gaia_mags  # Use the same values

        valid_gaia_mags = np.isfinite(matched_table[filter_band_to_use])
