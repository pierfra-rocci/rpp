                    ...
                    [
                        html.Div(
                            [
                                ...
                                dbc.Switch(
                                    id="apply-bias", label="Apply Bias", value=False
                                ),
                                dbc.Switch(
                                    id="apply-dark", label="Apply Dark", value=False
                                ),
                                dbc.Switch(
                                    id="apply-flat",
                                    label="Apply Flat Field",
                                    value=False,
                                ),
                            ...
                            [
                                ...
                                dbc.ButtonGroup(
                                    [
                                        ...
                                        dbc.Button(
                                            "VizieR",
                                            href="http://vizier.u-strasbg.fr/viz-bin/VizieR",
                                            target="_blank",
                                            color="secondary",
                                            size="sm",
                                        ),
                    ...
                    [
                        ...
                        html.Div(
                            [
                                ...
                                html.Div(
                                    id="science-image-container", className="mb-4"
                                ),
                                # Image statistics
...
def update_science_image(contents, filename):
    ...
    fig = go.Figure(
        data=go.Heatmap(
            z=data, colorscale="Viridis", colorbar=dict(title="Pixel Value")
        )
    )

    fig.update_layout(title=f"Science Image: {filename}", **PLOTLY_THEME["layout"])
        ...
        [
            ...
            dbc.CardBody(
                [
                    dbc.Row(
                        [
                            dbc.Col([html.H6("Mean"), html.P(f"{np.mean(data):.3f}")]),
                            dbc.Col(
                                [html.H6("Median"), html.P(f"{np.median(data):.3f}")]
                            ),
                            dbc.Col(
                                [html.H6("Std Dev"), html.P(f"{np.std(data):.3f}")]
                            ),
                        ]
                    )
                ]
...
def create_alert(message, color="primary", dismissable=True):
    ...
    return dbc.Alert(message, color=color, dismissable=dismissable, duration=4000)
...
def run_calibration(n_clicks, apply_bias, apply_dark, apply_flat):
    ...
    try:
        ...
        if apply_dark and dark_data is not None:
            exposure_ratio = science_header.get("EXPTIME", 1.0) / session.files_loaded[
                "dark_file"
            ][1].get("EXPTIME", 1.0)
        ...
        fig.update_layout(title="Calibrated Science Image", **PLOTLY_THEME["layout"])

        return (
            create_alert(
                f"Applied calibrations: {', '.join(steps_applied)}", "success"
            ),
...
def run_source_detection(n_clicks, seeing, threshold, mask_size):
    ...
    data = (
        session.calibrated_data
        if session.calibrated_data is not None
        else session.files_loaded["science_file"][0]
    )
    header = (
        session.calibrated_header
        if session.calibrated_header is not None
        else session.files_loaded["science_file"][1]
    )
    ...
    try:
        # Estimate background
        bkg = Background2D(
            data, box_size=100, filter_size=5, sigma_clip=SigmaClip(sigma=3.0)
        )
        ...
        daofind = DAOStarFinder(
            fwhm=mean_fwhm, threshold=threshold * bkg.background_rms.mean()
        )
        ...
        fig.update_layout(
            title=f"Detected Sources (N={len(sources)})", **PLOTLY_THEME["layout"]
        )
...
def solve_astrometry(n_clicks, api_key):
    ...
    if not api_key:
        return create_alert(
            "Please enter your Astrometry.net API key", "warning"
        ), dash.no_update
    ...
    try:
        ...
        if wcs_header is None:
            return create_alert(
                f"Astrometry.net failed after {max_tries} attempts", "danger"
            ), dash.no_update
        ...
        wcs_info = dbc.Card(
            [
                ...
                dbc.CardBody(
                    [
                        html.P(
                            f"Field center: RA = {ra_center:.6f}°, Dec = {dec_center:.6f}°"
                        ),
                        html.P(
                            f"Pixel scale: {wcs.wcs.cdelt[0] * 3600:.3f} arcsec/pixel"
                        ),
                    ]
                ),
            ]
        ...
        return (create_alert("Astrometric solution successful!", "success"), wcs_info)
...
@app.callback(
    ...
    [State("ra-input", "value"), State("dec-input", "value")],
)
def update_manual_coordinates(n_clicks, ra, dec):
    ...
    try:
        ...
        return create_alert(
            f"Coordinates updated: RA={ra_val}°, Dec={dec_val}°", "success"
        )
...
def run_catalog_matching(n_clicks, min_mag, max_mag, match_radius):
    ...
    if session.calibrated_header is None or "CRVAL1" not in session.calibrated_header:
        return None, create_alert(
            "WCS solution required before catalog matching", "warning"
        )
    ...
    try:
        ...
        pixels = wcs.all_world2pix(np.column_stack((catalog["ra"], catalog["dec"])), 0)
        ...
        fig.add_trace(go.Heatmap(z=data, colorscale="Viridis", showscale=True))
        ...
        fig.update_layout(title="GAIA Catalog Overlay", **PLOTLY_THEME["layout"])
        ...
        results_div = html.Div(
            [
                dcc.Graph(figure=fig),
                html.H5(
                    f"Found {len(catalog)} GAIA sources in field", className="mt-4"
                ),
...
def calculate_photometry_uncertainties(
    data, sources, phot_table, gain=1.0, readnoise=10.0
):
...
def run_zp_calibration(n_clicks, seeing):
    ...
    try:
        ...
        phot_table = perform_aperture_photometry(
            ...
            sources,
            seeing
            / abs(header.get("CDELT1", 1.0) * 3600.0),  # Convert seeing to pixels
        )
        ...
        results_div = html.Div(
            [
                ...
                html.H5(
                    "Calibrated Source Catalog (with uncertainties)", className="mt-4"
                ),
                dbc.Table.from_dataframe(
...
@app.callback(
    Output("download-data", "data"), [Input("download-results-btn", "n_clicks")]
)
