...
DIST_KEYS_PATTERNS = [
    r'^CPDIS', r'^DET2IM', r'^D?SS', r'^PV', r'^PVi_', r'^PS',  # distortion / pv / lookup hints
    r'^A_', r'^B_', r'^AP_', r'^BP_', r'^A_ORDER', r'^B_ORDER',  # SIP
    r'^CDELT[M]?', r'^CROT', r'^CDELTM'  # deprecated / odd keys (we only inspect these)
]


def list_interesting_keys(hdr):
    keys = ['CTYPE1','CTYPE2','CRPIX1','CRPIX2','CRVAL1','CRVAL2',
            'CDELT1','CDELT2','CD1_1','CD1_2','CD2_1','CD2_2','CROTA2',
            'WCSAXES','WCSNAME','WCSNAMEa']
    print("Core WCS keys:")
    for k in keys:
        if k in hdr:
            print(f"  {k:8s} = {hdr[k]}")
...
def main(path, hdu_idx=0, out_clean=None, remove_distortion=False):
    ...
    if remove_distortion:
        ...
        if w is not None:
            ...
            return 0
        hdul3.close()
    print("Unable to construct a valid WCS automatically. Inspect keys and try providing the referenced distortion table HDU (if any), or remove broken distortion keywords.")
    hdul.close()
    return 2
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: python check_fix_wcs.py input.fits [--hdu N] [--out cleaned.fits] [--remove-distortion]")
        sys.exit(1)
    path = sys.argv[1]
    hdu = 0
    out = None
    ...
    for a in sys.argv[2:]:
        if a.startswith('--hdu'):
            hdu = int(a.split('=')[1]) if '=' in a else int(sys.argv[sys.argv.index(a)+1])
        if a.startswith('--out'):
