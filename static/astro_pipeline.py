...
class ImageCalibrationPhotometryApp:
    ...
    def create_widgets(self):
        ...
        ttk.Label(file_frame, text="Science Image:").grid(
            row=0, column=0, padx=5, pady=5, sticky="w"
        )
        ttk.Entry(file_frame, textvariable=self.science_image_path, width=50).grid(
            row=0, column=1, padx=5, pady=5, sticky="ew"
        )
        ttk.Button(file_frame, text="Browse", command=self.load_science_image).grid(
            row=0, column=2, padx=5, pady=5, sticky="w"
        )

        ttk.Label(file_frame, text="Bias Frames Directory:").grid(
            row=1, column=0, padx=5, pady=5, sticky="w"
        )
        ttk.Entry(file_frame, textvariable=self.bias_frames_path, width=50).grid(
            row=1, column=1, padx=5, pady=5, sticky="ew"
        )
        ttk.Button(file_frame, text="Browse", command=self.load_bias_frames).grid(
            row=1, column=2, padx=5, pady=5, sticky="w"
        )

        ttk.Label(file_frame, text="Dark Frames Directory:").grid(
            row=2, column=0, padx=5, pady=5, sticky="w"
        )
        ttk.Entry(file_frame, textvariable=self.dark_frames_path, width=50).grid(
            row=2, column=1, padx=5, pady=5, sticky="ew"
        )
        ttk.Button(file_frame, text="Browse", command=self.load_dark_frames).grid(
            row=2, column=2, padx=5, pady=5, sticky="w"
        )

        ttk.Label(file_frame, text="Flat Frames Directory:").grid(
            row=3, column=0, padx=5, pady=5, sticky="w"
        )
        ttk.Entry(file_frame, textvariable=self.flat_frames_path, width=50).grid(
            row=3, column=1, padx=5, pady=5, sticky="ew"
        )
        ttk.Button(file_frame, text="Browse", command=self.load_flat_frames).grid(
        ...
        ttk.Checkbutton(
            calibration_frame, text="Apply Bias Correction", variable=self.apply_bias
        ).grid(row=0, column=0, padx=5, pady=5, sticky="w")

        self.apply_dark = tk.BooleanVar(value=True)
        ttk.Checkbutton(
            calibration_frame, text="Apply Dark Correction", variable=self.apply_dark
        ).grid(row=1, column=0, padx=5, pady=5, sticky="w")

        self.apply_flat = tk.BooleanVar(value=True)
        ttk.Checkbutton(
            calibration_frame,
            text="Apply Flat Field Correction",
            variable=self.apply_flat,
        ).grid(row=2, column=0, padx=5, pady=5, sticky="w")

        ttk.Button(
            calibration_frame, text="Calibrate Image", command=self.calibrate
        ...
        ttk.Label(photometry_frame, text="Aperture Radius (pixels):").grid(
            row=2, column=0, padx=5, pady=5, sticky="w"
        )
        ttk.Entry(photometry_frame, textvariable=self.aperture_radius, width=10).grid(
            row=2, column=1, padx=5, pady=5, sticky="w"
        )

        ttk.Label(photometry_frame, text="Inner Annulus Radius (pixels):").grid(
            row=3, column=0, padx=5, pady=5, sticky="w"
        )
        ttk.Entry(
            photometry_frame, textvariable=self.inner_annulus_radius, width=10
        ).grid(row=3, column=1, padx=5, pady=5, sticky="w")

        ttk.Label(photometry_frame, text="Outer Annulus Radius (pixels):").grid(
            row=4, column=0, padx=5, pady=5, sticky="w"
        )
        ttk.Entry(
            photometry_frame, textvariable=self.outer_annulus_radius, width=10
        ).grid(row=4, column=1, padx=5, pady=5, sticky="w")

        # PSF Photometry Parameters (Simplified)
        ttk.Label(photometry_frame, text="PSF Photometry (Simplified):").grid(
            row=5, column=0, padx=5, pady=5, sticky="w"
        )
        ttk.Label(
            photometry_frame, text="Assumes a Gaussian PSF for demonstration."
        ).grid(row=6, column=0, columnspan=2, padx=5, pady=5, sticky="w")

        ttk.Button(
            photometry_frame, text="Perform Photometry", command=self.perform_photometry
        ...
        ttk.Label(
            zeropoint_frame,
            text="Standard Star Coordinates (RA, Dec - comma separated) and Magnitudes (comma separated):",
        ).grid(row=0, column=0, columnspan=2, padx=5, pady=5, sticky="w")

        self.standard_star_info = tk.Text(zeropoint_frame, height=5, width=60)
        self.standard_star_info.grid(
            row=1, column=0, columnspan=2, padx=5, pady=5, sticky="w"
        )
        self.standard_star_info.insert(
            tk.END, "e.g., 10.68375, +41.26917, 9.5\n10.68583, +41.27139, 10.2"
        )

        ttk.Button(
            zeropoint_frame,
            text="Calculate Zero Point",
            command=self.calculate_zeropoint,
    ...
    def load_bias_frames(self):
        directory = filedialog.askdirectory(
            title="Select Directory Containing Bias Frames"
        )
        if directory:
    ...
    def load_dark_frames(self):
        directory = filedialog.askdirectory(
            title="Select Directory Containing Dark Frames"
        )
        if directory:
    ...
    def load_flat_frames(self):
        directory = filedialog.askdirectory(
            title="Select Directory Containing Flat Frames"
        )
        if directory:
    ...
    def calibrate(self):
        ...
        try:
            ...
            if self.apply_bias.get() and bias_dir:
                bias_fnames = [
                    f for f in tk.os.listdir(bias_dir) if f.endswith(".fits")
                ]
                if bias_fnames:
                    bias_list = [
                        CCDData.read(tk.os.path.join(bias_dir, fname), unit="adu")
                    ...
                    self.results_text.insert(
                        tk.END, "No bias frames found in the specified directory.\n"
                    )

            if self.apply_dark.get() and dark_dir:
                dark_fnames = [
                    f for f in tk.os.listdir(dark_dir) if f.endswith(".fits")
                ]
                if dark_fnames:
                    dark_list = [
                        CCDData.read(tk.os.path.join(dark_dir, fname), unit="adu")
                        for fname in dark_fnames
                    ]
                    master_dark = combine(dark_list, method="median", unit="adu")
                    science_image = subtract_dark(
                        science_image,
                        master_dark,
                    ...
                    self.results_text.insert(
                        tk.END, "No dark frames found in the specified directory.\n"
                    )

            if self.apply_flat.get() and flat_dir:
                flat_fnames = [
                    f for f in tk.os.listdir(flat_dir) if f.endswith(".fits")
                ]
                if flat_fnames:
                    flat_list = [
                        CCDData.read(tk.os.path.join(flat_dir, fname), unit="adu")
                    ...
                    self.results_text.insert(
                        tk.END, "No flat frames found in the specified directory.\n"
                    )
        ...
        try:
            if self.photometry_type.get() == "aperture":
                ...
                annulus_apertures = CircularAnnulus(
                    positions, r_in=inner_radius, r_out=outer_radius
                )
                ...
                phot_table["aperture_sum_bkgsub"] = phot_table["aperture_sum"] - (
                    bkg_mean * apertures.area
                )
                ...
                for row in phot_table:
                    self.results_text.insert(
                        tk.END,
                        f"Position: ({row['xcenter']:.2f}, {row['ycenter']:.2f}), Counts (Background Subtracted): {row['aperture_sum_bkgsub']:.2f}\n",
                    )

            elif self.photometry_type.get() == "psf":
                # Simplified PSF photometry using Gaussian PSF for demonstration
                mean, median, std = sigma_clipped_stats(
                    self.calibrated_image_data, sigma=3.0
                )
                ...
                if sources is not None:
                    ...
                    if detected_sources:
                        psf_photometry = PSFPhotometry(
                            psf_model, fitshape=9, finder=finder
                        )
                        ...
                        for row in result_table:
                            self.results_text.insert(
                                tk.END,
                                f"Position: ({row['x_0']:.2f}, {row['y_0']:.2f}), Flux: {row['flux_0']:.2f}\n",
                            )
                    else:
                        self.results_text.insert(
                            tk.END, "No sources found for PSF photometry.\n"
                        )
                else:
                    self.results_text.insert(
                        tk.END, "No sources found for PSF photometry.\n"
    ...
    def calculate_zeropoint(self):
        if self.calibrated_image_data is None:
            messagebox.showerror(
                "Error", "Please calibrate the image first and perform photometry."
            )
            return

        standard_star_info = (
            self.standard_star_info.get("1.0", tk.END).strip().split("\n")
        )
        if not standard_star_info or not any(standard_star_info):
            messagebox.showerror(
                "Error", "Please enter standard star coordinates and magnitudes."
            )
            return
        ...
        try:
            ...
            if not standard_stars:
                messagebox.showerror(
                    "Error", "No valid standard star information provided."
                )
                return
            ...
            for row in phot_table:
                ...
                exposure_time = (
                    1.0  # Assuming 1 second exposure for simplicity, adjust if needed
                )
                if counts > 0:
                    ...
                    self.results_text.insert(
                        tk.END,
                        f"Warning: Zero or negative counts for a standard star.\n",
                    )
                    return
            ...
            if len(instrumental_magnitudes) != len(standard_magnitudes):
                messagebox.showerror(
                    "Error",
                    "Number of detected standard stars does not match the input.",
                )
                return

            zero_points = np.array(standard_magnitudes) - np.array(
                instrumental_magnitudes
            )
            ...
            for i in range(len(standard_magnitudes)):
                self.results_text.insert(
                    tk.END,
                    f"Standard Star {i + 1}: Standard Mag = {standard_magnitudes[i]:.2f}, Instrumental Mag = {instrumental_magnitudes[i]:.2f}, Zero Point = {zero_points[i]:.2f}\n",
                )
            self.results_text.insert(
                tk.END, f"Mean Zero Point: {mean_zero_point:.2f}\n"
            )

        except ValueError:
            messagebox.showerror(
                "Error",
                "Invalid format for standard star information. Please use 'RA, Dec, Magnitude' per line.",
            )
        except Exception as e:
